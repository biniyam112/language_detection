...
Since the trained models and sammple audios are included in this repository, 
...
# 2. If samples/predict_test/ is missing, create a small FLEURS sample set
...
# 6. Train and evaluate the LSTM comparison model
python src/train.py --model lstm
python src/evaluate.py --model lstm

# 7. Generate or refresh sample files from the local held-out split
python generate_sample.py --source fleurs --samples-per-language 5

# 8. Run sample predictions for each model
python run_sample_predictions.py --model cnn
python run_sample_predictions.py --model lstm


# 9. For live audio testing execute /notebooks/microphone_language_prediction.ipynb
```
