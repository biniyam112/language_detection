...
# 2. Generate a small FLEURS sample set for testing
python generate_sample.py --source fleurs --samples-per-language 5

# 3. Run the saved CNN checkpoint on generated sample audio
python run_sample_predictions.py --model cnn

# 4. Run the saved LSTM checkpoint on the same samples
python run_sample_predictions.py --model lstm

# 5. Predict one audio file with a selected model
python src/predict.py --audio path/to/audio.wav --model cnn

# 6. For live audio testing execute /notebooks/microphone_language_prediction.ipynb
```
